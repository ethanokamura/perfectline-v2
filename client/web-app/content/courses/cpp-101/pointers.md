...
By incrementing `str` with `++str` we are increasing the value of the pointer/memory address `str` by the size of one char. Meaning we move forward to the next item in the referenced string.
...
2. Rarely! (You should use `std::vector` instead of arrays most of the time)
