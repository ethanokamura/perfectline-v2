...
## Doubly Linked List
Each list element (i.e., node) contains a pointer to the next node in the list, along with the data each node holds.

Here's a basic visual for a linked list!

![](/svg/linked-list.svg)

The `LinkedList` class is an archetypal doubly linked list where `Node` structures and internal list details are kept private. Also, note that all member functions are defined inside the class declaration (no separate .h and .cpp/.cc files). Template classes are usually defined this way. For this class, the parameter `T` is the type of the list elements. You can think of it as a dynamic or generic way to hold data!
